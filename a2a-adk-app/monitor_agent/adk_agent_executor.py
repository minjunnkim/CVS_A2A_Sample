...
class MonitorAgentExecutor(AgentExecutor):
    """Executes monitoring tasks and reports code execution status."""
    ...
    async def _process_request(self, new_message: types.Content, session_id: str, task_updater: TaskUpdater):
        ...
        missing_target_prompted = False
        async for event in self._run_agent(session_id, new_message):
            if event.is_final_response():
                parts = convert_genai_parts_to_a2a(event.content.parts)
                # Check if monitoring failed due to missing target and prompt user
                if any("target" in (getattr(p, 'text', '') or '').lower() and "missing" in (getattr(p, 'text', '') or '').lower() for p in parts):
                    if not missing_target_prompted:
                        task_updater.add_artifact([
                            TextPart(text="Please specify the system or component you want to monitor or report an issue for.")
                        ])
                        missing_target_prompted = True
